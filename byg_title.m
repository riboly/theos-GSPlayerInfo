...
#import <mach/mach.h>
// iOS 公开 SDK 不提供 mach_vm.h，用 vm_*_64 / vm_read_overwrite
#import <pthread.h>
#import <stdlib.h>
#import <string.h>
#import <sys/types.h>
...
static bool byg_read_chunk(vm_address_t addr, vm_size_t size, uint8_t *buf) {
    vm_size_t out = 0;
    kern_return_t kr = vm_read_overwrite(
        mach_task_self(), addr, size, (vm_address_t)(uintptr_t)buf, &out);
...
static int byg_count_exact(NSString *text, byg_region_t *regions, size_t nreg, uint8_t *buf,
    ...
    for (size_t ri = 0; ri < nreg && cnt < 20; ri++) {
        if (CFAbsoluteTimeGetCurrent() > deadline)
            break;
        vm_address_t cur = regions[ri].addr;
        vm_size_t remain = regions[ri].size;
        // 与 Frida 一致：单区最多扫 8MB
        if (remain > 8ull * 1024 * 1024)
            remain = 8ull * 1024 * 1024;

        while (remain > 0 && cnt < 20) {
            vm_size_t chunk = remain > chunk_cap ? (vm_size_t)chunk_cap : remain;
            ...
            if (remain <= chunk)
                break;
            // overlap 推进
            vm_size_t step = chunk > OVERLAP ? chunk - (vm_size_t)OVERLAP : chunk;
            cur += step;
            remain -= step;
        }
    }
...
static int byg_scan_current_title_utf8(char **out_utf8, uint32_t timeout_ms) {
    ...
    *out_utf8 = NULL;
    ...
    for (size_t i = 0; i < max_reg; i++) {
        ...
        vm_address_t cur = regions[i].addr;
        vm_size_t remain = regions[i].size;
        ...
        while (remain > 0) {
            if (CFAbsoluteTimeGetCurrent() > deadline)
                break;
            vm_size_t chunk = remain > CHUNK ? (vm_size_t)CHUNK : remain;
            if (!byg_read_chunk(cur, chunk, buf)) {
                cur += chunk;
                remain = remain > chunk ? remain - chunk : 0;
                continue;
